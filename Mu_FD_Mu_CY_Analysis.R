...
## Trait indices (CWM and D). Mixtures' CWMs and Ds are computed based on single-variety trait values (see Material & Methods in the main text)

trait_data <- read.csv(text = getURL("https://raw.githubusercontent.com/germmtz/multifaceted_FD_multifaceted_YLD/master/CWM_D.csv"), sep=";",header=T)
...
######################
#### PAIWISE CORRELATIONS BETWEEN MONOCULTURE TRAITS (Supplementary Figure 4) ######
#####################

## Renaming variables
colnames(traitM) <- c(":Angle[aer]", ":Angle[root]",":Diam[sem]",":SRL[sem]",":RTD[sem]",":RBI[sem]",":RLD[sem]",":Diam[adv]",":SRL[adv]",":RTD[adv]",":RBI[adv]",":RLD[adv]","Till. nb.","Ear. bio.","SLA","LNC","Height","Heading","Maturity")
...
##
hist(perfP[,"RYT_GPC"], main="Grain Protein\nContent (GPC)", xlab=expression("RYT"["GPC"]), ylab="Nb of observations", col="grey", border="white", axes=F, xlim=c(0,2), ylim=c(0,120),breaks=seq(0,2,by=0.05), cex.lab=1.3, cex.main=1.5)
...
effect_sizes <- effect_sizes[order(gsub("^CWM|^D","",row.names(effect_sizes)), decreasing = F),]
...
axis(2, las=1, labels=c("Heading", "LNC", expression("RBI"["sem"]), expression("RTD"["adv"]), "Till. nb.", expression("Diam"["adv"]), expression("Diam"["sem"]),"Maturity", expression("Angle"["aer"]), "Ear. bio.", "Till. nb.", expression("Diam"["sem"])), at=effect_sizes$y, tick=F, cex.lab=1.3, cex.axis=1.3) 
...
arrows(effect_sizes$Estimate,effect_sizes$y,effect_sizes$lb,effect_sizes$y, code=2, angle=90, length=0.03, col=colvec, lwd=2.5)
points(effect_sizes$Estimate, effect_sizes$y, pch=21, bg=bgvec, col=colvec, cex=1.6, lwd=1.5)
axis(2, las=1, labels=c("Ear. bio.", expression("RBI"["sem"]), expression("RTD"["adv"]),"Till. nb.", "Till. nb.", expression("Diam"["sem"]), "Heading", expression("Diam"["adv"]), expression("RTD"["adv"]), expression("SRL"["adv"]), "LNC", "Maturity", "SLA"), at=effect_sizes$y, tick=F, cex.lab=1.3, cex.axis=1.3) 
...
mtext(expression(bold("Grain Protein Content RYT (RYT"["GPC"]*")")), side=3, line=3, at=1.3, font=2, cex=1.7)
R2 <- round(mean(info_model$R2_adj),2)
mtext(bquote(bar(R²[adj])*" = "*.(R2)), side=3, line=0.8, at=1.3, font=2, cex=1.5)
...
#################
### Analysis of Spike Number per m² (SNb) and Thousand Kernel Weight (TKW) (Supplementary Figure 6) ##
#################
...
axis(2, las=1, labels=c("Heading", "Maturity", expression("RBI"["sem"]), expression("RTD"["adv"]),  expression("Angle"["aer"]), expression("Diam"["sem"]), "Height", "Till. nb.", "LNC", expression("RLD"["adv"]), "SLA"), at=effect_sizes$y, tick=F, cex.lab=1.3, cex.axis=1.3) 
...
#########################################
############ Plot legend (main text - Figure 3, Supplementary Figures 6) ################
#########################################
...
legend(xlegend+0.02+0.3,ylegend-1.2, legend="D", bty="n", xpd=NA, cex=1.4)
...
## plotting the clustering tree with a delimitation of the 3 functional groups (Supplementary Figure 3)
...
abline(h=1, lty=2, lwd=2)
mtext(side=3, line=2, "Grain Protein\nContent RYT", font=2, cex=size_main, at=3.5)
